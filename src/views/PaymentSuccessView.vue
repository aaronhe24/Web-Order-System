<template>
    <div class="success-wrap">
      <div class="success-container">
        ...
        <div>
          <h2>{{$t('payment_result.success_title')}}</h2>
          <br>
          <h5>{{$t('payment_result.order_num')}}: {{orderNum}}</h5>
          <br>
          <p>{{$t('payment_result.success_desc')}}</p>
          <br>
          <div class="btn_group">
            <button class="order_detail_btn" @click="clickOrderDetail">{{$t('payment_result.order_detail')}}</button>
...
<script lang="ts">
  ...
  @Component
  export default class PaymentSuccessView extends Vue {
    ...
    mounted() {
      ...
      this.$gtag.event('conversion',
        {
          'send_to': 'AW-919930352/Kh2kCPe41O8BEPCL1LYD',
          'value': 1.0,
          'currency': 'CAD',
          'transaction_id': ''
        })
